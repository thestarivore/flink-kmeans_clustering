...
read -p "Num of file with points: " POINTS_FILES
read -p "Min points: " MIN_POINTS
...
points=$MIN_POINTS
for j in $(seq 1 $POINTS_FILES);
do
	echo "---STARTING with $points points"
	...
	do
		echo "Start execution num $i with file $BASE_PATH/input/points_$points.csv"
	   	RESULT=$(flink run -p $PARALLELISM -q ../flink-project-1.7.0.jar -numcentroids $CENTROIDS -recompnearest $RECOMP_NEAREST -iterations $ITERATIONS -custconvergence $CUSTOM_CONV -points "$BASE_PATH/input/points_$points.csv" -centroids "$BASE_PATH/input/$j/centroids_$i.csv" -pointsout "$BASE_PATH/output/$j/new_points_$i.csv" -centroidsout "$BASE_PATH/output/$j/new_centroids_$i.csv" -objfunout "$BASE_PATH/output/$j/objfun_$i.csv")
	   	echo "$RESULT"
